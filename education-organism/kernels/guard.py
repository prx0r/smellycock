...
def _normalise(s: str) -> str:
    """NFKC fold + strip punctuation/whitespace + diacritic fold + lowercase.

    The diacritic fold (NFKD + remove combining marks) collapses Sanskrit transliteration variants so
    `Tantrāloka` matches `Tantraloka` and `Śiva` matches `Siva` — preventing a REAL quote from being
    false-flagged just because the answer used a macron the source didn't (or vice-versa). This is the
    deterministic, script-independent analogue of fojin's CJK 繁→简 fold."""
    s = unicodedata.normalize("NFKC", s)
    s = unicodedata.normalize("NFKD", s)          # decompose: macrons/dots become base + combining mark
    s = "".join(c for c in s if not unicodedata.combining(c))   # drop the combining marks
