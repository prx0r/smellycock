...
## 6. TRANSLATION-INTEGRATION ENDPOINTS (the live Pāṭala translation surface)

Served as compiled bytes, ETag/304 + immutable (perf doctrine):

| Endpoint | Returns |
|---|---|
| `GET /openpatala/translation` | every work's T1/L2/C1 ledger status + `next_action` + committed layer counts |
| `GET /openpatala/translation/{work_id}` | one work's record; `?select=work_id,committed` |
| `GET /openpatala/translation/{work_id}/content` | the committed **translation CONTENT** — T1 glosses, L2 readings, L200 audits, C1 commentaries (`?select=C1`) |
| `GET /openpatala/status` | the **live ops board** — active processes (RSS/elapsed), per-layer counts, queue, plan, logs |
| `GET /works/{work_id}?select=translation` | the bibliography record's `translation` block (committed counts + content) |

MCP: `get_translation_status` · `get_translation_status_for_work` · `patala_get_ops_status` ·
`patala_next_action` · `patala_get_work_state` · `patala_get_translation_progress`.
